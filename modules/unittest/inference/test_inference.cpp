...
TEST_F(ModelLoaderTest, Create) {
  ...
  ASSERT_TRUE(model_loader_->Init(model_path));
...
TEST_F(ModelLoaderTest, Run) {
  ...
  uint32_t channel_size = model_loader_->get_channel_size();
  uint32_t height = model_loader_->get_height_size();
  uint32_t width = model_loader_->get_width_size();
...
TEST_F(ModelLoaderTest, PostProcess) {
