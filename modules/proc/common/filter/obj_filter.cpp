...
std::vector<int> ObjFilterCommon::ParseIntList(const std::string& raw) {
  ...
  // Whitespace around tokens and extra/leading/trailing commas are tolerated.
std::vector<std::string> ObjFilterCommon::ParseStringList(const std::string& raw) {
  return SplitCsv(raw);
}
...
bool ObjFilterCommon::ParsePosition(const std::string& raw, AxisRange* x_range, AxisRange* y_range) {
...
bool ObjFilterCommon::ParseAxisRange(const std::string& body, AxisRange* range) {
  *range = AxisRange{};
...
int ObjFilterCommon::ParseBoundToken(const std::string& token, float* value) {
