 ...
 public:
  bool Init(const std::map<std::string, std::string> &params) override {
    ...
    if (config_file_.empty()) {
      LOGW(POSTPROC) << "Init config_file is empty.";
      return true;
    }
  ...
  int Execute(const std::vector<float*>& outputs, ModelLoader* model,
    ...
    int num_classes = int(output_shape.numel() / output_shape.N());
...
class Post_Resnet : public Postproc {
 public:
  bool Init(const std::map<std::string, std::string> &params) override {
  ...
  int Execute(const std::vector<float*>& outputs, ModelLoader* model,
    ...
    class_info.id = best_class;
    class_info.name = class_name;
    class_info.model_name = model_name_;
    class_info.score = best_score;
    class_info.value = best_score;

    obj->classes.push_back(class_info);

    {
      ...
      objs_holder->objs_.insert(objs_holder->objs_.end(), obj);
    }
 ...
 private:
  DECLARE_REFLEX_OBJECT_EX(Post_Resnet, cnstream::Postproc);
};  // class Post_Resnet

IMPLEMENT_REFLEX_OBJECT_EX(Post_Resnet, cnstream::Postproc);
