...
/**
 * @brief PPOCRv6 识别后处理（CTC 贪婪解码）
 *
 * 与 PaddleOCR CTCLabelDecode（ppocr/postprocess/rec_postprocess.py）对齐：
 *   每个时间步 argmax → 跳过 blank(索引 0) → 合并连续重复索引 → 置信度取保留字符概率均值。
 * 字典拼装顺序（同 BaseRecLabelDecode.__init__，rec_postprocess.py:41-47）：
 *   [0] blank + [1..N] label 文件字符 + [N+1] 空格，PP-OCRv6 下 N=18708，合计 18710 类。
 */
class Post_PPOCRv6_rec_Obj : public ObjPostproc {
 public:
  bool Init(const std::map<std::string, std::string> &params) override {
    ...
    LOGI(POSTPROC) << "PPOCRv6 post conf file: " << config_file_;
    ...
    LOGI(POSTPROC) << "PPOCRv6 label_path: " << label_path_;
    ...
    if (!label_path_.empty()) {
        ...
        label_list_.insert(this->label_list_.begin(), "#");  // blank, index 0
        label_list_.push_back(" ");  // space, last index
        LOGI(POSTPROC) << "PPOCRv6 dict size (with blank & space): " << label_list_.size();
    }
  ...
  int Execute(const std::vector<float*>& outputs, ModelLoader* model,
    ...
    if (output_shape.ndims() != 3) {
        LOGE(POSTPROC) << "PPOCRv6 output shape " << output_shape;
        return -1;
    }
    if (output_shape.shape(0) != 1) {
        LOGE(POSTPROC) << "PPOCRv6 model shape must be batch size 1, but " << output_shape;
        return -1;
    }

    uint32_t rows = output_shape.shape(1);  // 40 (静态宽度 320 时)
    uint32_t cols = output_shape.shape(2);  // 18710

    // 字典与模型输出类别数强校验：不匹配说明 label 文件与 engine 不配套，
    if (label_list_.size() != static_cast<size_t>(cols)) {
        LOGE(POSTPROC) << "dict/model mismatch: label_list size " << label_list_.size()
                       << " (label_path: " << label_path_ << ") != output cols " << cols;
        return -1;
    }
    ...
    if (!frame) {
        LOGE(POSTPROC) << "PPOCRv6: DataFrame is null";
        return -1;
    }
    ...
    if (debug_saver_.enable())  {
      ...
      if (safe_roi.width > 0 && safe_roi.height > 0) {
        debug_saver_.MaybeSave("post_ocrv6_rec_crop", img, str_res,
    ...
    LOGD(POSTPROC) << "PPOCRv6: " << dr_ms << " ms, result: " << str_res;
 ...
 private:
  ...
  DECLARE_REFLEX_OBJECT_EX(Post_PPOCRv6_rec_Obj, cnstream::ObjPostproc);
};  // class Post_PPOCRv6_rec_Obj

IMPLEMENT_REFLEX_OBJECT_EX(Post_PPOCRv6_rec_Obj, cnstream::ObjPostproc);
